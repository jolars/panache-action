...
$verify = if ($env:PANACHE_VERIFY_CHECKSUM) { $env:PANACHE_VERIFY_CHECKSUM } else { 'true' }
...
try {
    ...
    if ($verify -eq 'true') {
        # Verify against the release's SHA256SUMS manifest. Releases that
        # predate the manifest 404 here, in which case we warn and continue.
        $expected = $null
